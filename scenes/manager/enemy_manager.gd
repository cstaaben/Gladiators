...
func get_spawn_position():
	...
	# use raycast to check for collisions between player and spawn position
	var params = PhysicsRayQueryParameters2D.create(player.global_position, spawn_position, 1 << 0)
	var result = get_tree().root.world_2d.direct_space_state.intersect_ray(params)
	if result.is_empty():
		# no collision
		return spawn_position
	else:
		# collision, rotate 90 and try again
