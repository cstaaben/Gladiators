...
func collect():
	...
	var player_health_component = player.health as HealthComponent
	if player_health_component.current == player_health_component.max_health:
		return
	
	player_health_component.heal(2)
